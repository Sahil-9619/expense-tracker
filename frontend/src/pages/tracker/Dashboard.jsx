...
export default function Dashboard({
  ...
  const chartData = useMemo(() => {
    ...
    const outOfRangeTx = transactions.filter(
    ...
    const labels = svgPoints.filter(
      (_, idx) => idx % labelStep === 0 || idx === points.length - 1,
    );
  ...
  const assetTotal = assetDistribution.reduce(
    (sum, asset) => sum + Math.abs(asset.amount),
    0,
  );

  const cardBase =
    "relative rounded-sm border border-[var(--card-border)] p-4 flex flex-col bg-[var(--card-bg)] transition-all duration-200 hover:shadow-lg hover:shadow-black/20";
      ...
      <div className="max-w-7xl mx-auto flex flex-col gap-4">
        ...
        <div className="flex items-end justify-between px-2">
          <div>
            ...
            <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase tracking-widest opacity-60">
        ...
        <div className="grid grid-cols-12 gap-3">
          ...
          />
          <div
            className={`${cardBase} col-span-12 sm:col-span-6 lg:col-span-3 group`}
          >
          ...
          >
            ...
            <div className="w-full h-1.5 bg-[var(--bg-color)] rounded-full mt-auto mb-1 overflow-hidden border border-[var(--card-border)]">
              <div
                className="h-full bg-gradient-to-r from-emerald-400 to-emerald-500 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)]"
          ...
          <div
            className={`${cardBase} col-span-12 lg:col-span-6 min-h-[380px] flex flex-col`}
          >
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--text-primary)]">
                Cash Flow Analysis
              </h2>
              ...
              {chartData.labels.length > 0 && (
                <div className="flex justify-between mt-2 pt-2 border-t border-[var(--card-border)]/30">
                  {chartData.labels.map((lbl, idx) => (
                    <span
                      key={idx}
                      className="text-[8px] font-black text-[var(--text-secondary)] uppercase tracking-wider"
                    >
                      {lbl.label}
                    </span>
                  ))}
          ...
          <div className={`${cardBase} col-span-12 lg:col-span-6`}>
            <div className="flex justify-between items-center mb-3">
              ...
              <Link
                ...
                className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest text-[var(--text-secondary)] hover:text-emerald-500 transition-all"
              >
                View All
              </Link>
            </div>
                ...
                <tbody className="text-[10px]">
                  {sortedTransactions.length === 0 ? (
                    <tr>
                      <td
                        colSpan="4"
                        className="py-10 text-center text-[var(--text-secondary)] font-bold uppercase tracking-widest opacity-40"
                      >
                    ...
                    sortedTransactions.map((tx) => {
                      const config = categoryConfig[tx.category] ||
                        categoryConfig["Other"] || { accent: "slate" };
                      return (
                        <tr
                          key={tx.id}
                          className="border-b border-[var(--card-border)]/50 last:border-0 group transition-colors hover:bg-[var(--bg-color)]/50"
                        >
                          <td className="py-2.5">
                            <div className="flex flex-col">
                              <span className="font-black text-[var(--text-primary)] uppercase tracking-tighter">
                                {tx.merchant || tx.description}
                              </span>
                              <span className="text-[9px] text-[var(--text-secondary)] opacity-60 font-medium">
                          ...
                          <td className="py-2.5">
                            <span
                              className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest border border-${config.accent}-500/20 bg-${config.accent}-500/5 text-${config.accent}-400`}
                            >
                              {tx.category}
                            </span>
                          </td>
                          <td
                            className={`py-2.5 text-right font-mono font-black tracking-tighter ${tx.type === "income" ? "text-emerald-500" : "text-[var(--text-primary)]"}`}
                          >
                            {tx.type === "income" ? "+" : ""}₹
                          ...
                          <td className="py-2.5 text-center">
                            <div
                              className={`w-1.5 h-1.5 rounded-full mx-auto ${tx.type === "income" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-slate-500"}`}
                            ></div>
                          </td>
                        </tr>
                      );
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 mt-auto pt-3">
        ...
        <span
          className={`text-[9px] font-black uppercase tracking-widest ${isUp ? "text-emerald-500" : "text-rose-500"}`}
        >
          {trend} vs last month
        </span>
      </div>
