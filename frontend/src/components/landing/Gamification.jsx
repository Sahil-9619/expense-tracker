    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row items-end justify-between mb-24 gap-8">
        <div>
          ...
          <h3 className="text-5xl md:text-7xl font-black tracking-tighter uppercase text-white font-display">Gamify Your<br />Wealth Journey.</h3>
        </div>
        ...
        {[
          ...
          <ThreeDCard key={i}>
            <div className="flex justify-between items-start mb-10">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 border border-white/10 px-3 py-1.5 rounded-lg bg-white/5">
                {tier.unlocked ? 'Unlocked' : 'Locked'}
              </span>
            </div>

            <h4 className="text-2xl font-black text-white uppercase tracking-tight mb-2 font-display">{tier.name}</h4>
            ...
            <div className="space-y-3 pt-6 border-t border-white/10">
              {tier.perks.map(perk => (
                <div key={perk} className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{perk}</span>
                </div>
              ))}
            </div>
          </ThreeDCard>
